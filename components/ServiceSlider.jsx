		...
		>
			{serviceData.map((item, i) => (
				<SwiperSlide key={i} className="pt-4 pb-12">
					<div className="relative h-full bg-slate-50/50 dark:bg-black/40 backdrop-blur-sm border border-slate-200 dark:border-white/5 hover:border-sky-500/30 dark:hover:border-red-500/30 rounded-2xl p-6 sm:p-8 flex flex-col gap-y-4 group cursor-pointer transition-all duration-300 hover:-translate-y-2 hover:shadow-[0_10px_30px_rgba(14,165,233,0.15)] dark:hover:shadow-[0_10px_30px_rgba(239,68,68,0.15)] overflow-hidden">
						{/* Ambient Hover Glow */}
						<div className="absolute -inset-full bg-gradient-to-tr from-transparent via-slate-200/50 dark:via-white/5 to-transparent group-hover:animate-[shimmer_1.5s_infinite] pointer-events-none" />
						...
						<div className="flex justify-between items-start mb-2">
							...
							<div className="w-10 h-10 rounded-full bg-slate-200/50 dark:bg-white/5 flex items-center justify-center -translate-y-2 translate-x-2 opacity-0 group-hover:opacity-100 group-hover:translate-y-0 group-hover:translate-x-0 transition-all duration-300 group-hover:bg-sky-500/20 dark:group-hover:bg-red-500/20">
						...
						<div className="flex flex-col mt-auto">
							...
							<p className="text-sm leading-relaxed text-slate-600 dark:text-gray-400 group-hover:text-slate-800 dark:group-hover:text-gray-300 transition-colors duration-300">
								{item.description}
							</p>
						</div>
